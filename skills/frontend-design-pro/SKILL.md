...
**`frontend-design-pro`** owns **anti-slop visual design patterns** — distinctive typography choices, intentional color systems, spatial composition, and motion choreography for web interfaces. **`motion-design-pro`** owns deep animation logic with GSAP/Framer Motion timelines when motion is the primary concern. **`ui-stack-pro`** owns design token enforcement and 8px grid/•••• rules. **`shadcn-mastery-pro`** owns shadcn/ui component primitives and Radix-based composition.
